...
네！ 모든 분석 명령에 `--lang` 옵션을 사용할 수 있습니다：
...
지원 언어：`zh-TW`（繁體中文, 기본값）、`en`（English）、`ko`（한국어）.

### Q：한국어 논문도 분석할 수 있나요？

물론입니다！ AI가 논문의 언어를 자동으로 감지하므로 한국어, 영어 논문 모두 지원됩니다. 언어가 혼합된 논문들을 함께 분석하는 것도 가능합니다. 출력 언어는 `--lang` 옵션으로 제어되며 논문 자체의 언어와는 관계없습니다.
